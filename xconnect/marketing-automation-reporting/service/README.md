...
## Building the Marketing Automation Reporting Locally
...
|`C:/Certificates`                    | Mount   | Yes | A directory mounted to `C:/Certificates` that contains two pfx files, one named `root-cert`, another named `xconnect-client` and the other that matches the value in `$HOST_NAME` |
...
|`XCONNECT_COLLECTION_SEARCH_SERVICE` | Env Var | Yes | The xconnect collection search service. |
|`XCONNECT_CLIENT_CERT_PASSWORD`      | Env Var | Yes | The export password for the xconnect client cert. |
