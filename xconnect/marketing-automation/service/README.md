...
## Building the Marketing Automation Locally
...
|`C:/Certificates`               | Mount   | Yes | A directory mounted to `C:/Certificates` that contains three pfx files, one named `root-cert`, another named `xconnect-client` and the other that matches the value in `$HOST_NAME` |
...
|`XCONNECT_CLIENT_CERT_PASSWORD` | Env Var | Yes | The export password for the website cert. |
